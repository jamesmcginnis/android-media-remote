...
- ```
  ...
  ```
- 
- 1. androidtv / Fire TV  (legacy)  — androidtv.adb_command with
- ```
  "input keyevent N" syntax.  Needs ADB debug enabled on the TV.
  ```
- 
- 1. media_player fallback — for actions HA can handle natively
- ```
...
```
    ...
    /* Outlined icon buttons — shuffle / repeat, same base aesthetic as nav buttons */
    .md-icon-btn {
    ...
    .md-icon-btn svg { width: 20px; height: 20px; fill: rgba(255,255,255,0.82); transition: fill 0.18s ease; }
    .md-icon-btn:active, .md-icon-btn.pressed { background: rgba(255,255,255,0.14); transform: scale(0.93); }
    /* Active (on) state — accent tint signals shuffle/repeat is enabled */
    .md-icon-btn.active { background: rgba(66,133,244,0.15); border-color: rgba(66,133,244,0.30) !important; }
    ...
    /* Volume +/− buttons (vol-btn-mode) — same outlined aesthetic as nav buttons */
    .vol-btn {
    ...
    .vol-btn svg { width: 20px; height: 20px; fill: rgba(255,255,255,0.82); }
