...
class AndroidMediaRemote extends HTMLElement {
  ...
  _sendPowerCommand() {
    ...
    if (isOff) {
      /* For power ON: call media_player.turn_on first — works for Cast (WoL/keep-alive)
       * and also send KEYCODE_WAKEUP via remote for Android TV integration.
       * Both fire simultaneously — whichever path works wins. */
      this.call('turn_on');
      if (hasRem) {
        this._hass.callService('remote', 'send_command', {
          entity_id: remId, command: 'KEYCODE_WAKEUP'
        }).catch(() => {
          this._hass.callService('androidtv', 'adb_command', {
            entity_id: this._entity, command: 'input keyevent 224'
          }).catch(() => {});
        });
      }
    } else {
      /* For power OFF: keycode first (cleaner shutdown), turn_off as fallback */
      if (hasRem) {
        this._hass.callService('remote', 'send_command', {
          entity_id: remId, command: 'KEYCODE_POWER'
        }).catch(() => {
          this._hass.callService('androidtv', 'adb_command', {
            entity_id: this._entity, command: 'input keyevent 26'
          }).catch(() => this.call('turn_off'));
        });
      } else {
        this.call('turn_off');
      }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .vol-btn:active, .vol-btn.pressed { background: rgba(255,255,255,0.14); transform: scale(0.93); }
        .volume-slider {
  ...
  setupListeners() {
    ...
    const openApps = () => {
      const state   = this._hass.states[this._entity];
      /* source_list is standard; some integrations use app_list */
      const sources = state?.attributes?.source_list
        || state?.attributes?.app_list
        || [];
  ...
  updateContent(state) {
    ...
    /* ── Track info ─────────────────────────────────────────────
     * Chromecast and some integrations drop media_title between
     * polling cycles while still reporting state === 'playing'.
     * We cache the last known good values and only clear them when
     * the source changes or playback genuinely stops.
     */
    const currentSource = state.attributes.source || state.attributes.app_name || '';
    if (currentSource !== this._lastMeta.source) {
      /* Source changed — invalidate the cache */
      this._lastMeta = { title: null, artist: null, source: currentSource };
    }

    /* Rich title fallback: media_title → media_series_title → app_name → cached → 'Playing'/'Idle' */
    const freshTitle = state.attributes.media_title
      || state.attributes.media_series_title
      || (isPlaying ? state.attributes.app_name : null);
